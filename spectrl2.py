...
def site_arguments(spectral_path, loc, dtadir):
    ...
    '''
    solcast_tmy = glob.glob(pathname='*.csv', root_dir= spectral_path / loc / 'Resource/Solcast')[0]
...
def spectral2_arguments(tmy, tmy_solpos, tracker_config):
    tracker_angles = pvl.tracking.singleaxis(tmy_solpos.apparent_zenith,
                                         tmy.azimuth,
    ...
    tmy_relhum = pvl.atmosphere.get_relative_airmass(tmy_solpos.apparent_zenith,
